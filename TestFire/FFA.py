...
import math
import time
from TestFire.solution import solution
...
def FFA(objf,lb,ub,dim,n,MaxGeneration):
    ...
    zn=numpy.ones(n) #n number of fireflies
    zn.fill(float("inf")) #zn array of fireflies
    
    
    #ns(i,:)=Lb+(Ub-Lb).*rand(1,d);
    ns=numpy.random.uniform(0,1,(n,dim)) *(ub-lb)+lb # ns firefly matrix with random numbers for locations of each firefly
    Lightn=numpy.ones(n) # lightn is the object function
    ...
    for k in range (0,MaxGeneration):     # start iterations
        ...
        for i in range(0,n):
            zn[i]=objf(ns[i,:]) # send the fireflys location to the objective function
            Lightn[i]=zn[i]
